...
| 项 | 内容 |
|---|---|
| 状态 | **前置门草案（待拍板）**——本文件列出裁决点与默认推荐，owner 拍板后转定稿进入执行轮 |
| 日期 | 2026-08-28 |
...
- [ ] 五项裁决点均有明确拍板记录（本文件转定稿）
- [ ] 基准工具入库并附基线数字（docstring 续记惯例）＋ InsightFace 多线程安全性实测结论
- [ ] 并行化落地且等价性对照测试通过
- [ ] 持久化批量下推，往返 O(N)→O(1)
- [ ] 性能目标达成（阈值待 A-1 基线数据出台后由 owner 定，建议起点：4 workers 下 ≥2× 串行）
- [ ] 全量质量门绿：ruff / mypy / pytest 全量 + CI 三平台
