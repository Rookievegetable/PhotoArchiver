...
| Next Step | 后续候选项均未排期：M8「可发布」里程碑候选（License 落定、首个语义化 tag 与发布工程、user-guide 补齐、macOS 抽验——待负责人拍板主线）；下一主版本评审 `enable(context)` 兼容路径移除；性能加固；export 写能力单独裁决；宿主审批门（如真实高危写用例出现）。 |
