...
当前 HEAD：v2.3.1 补丁轮提交（feat + chore(release) + docs(status)，见 §5）——补丁内容（`65cfa26` 起）落在已发布的 v2.3.1（tag 现→`39c0ca7`，Release published 2026-09-05T14:23:43Z）之后，经 owner 裁决以 tag 重打折入 v2.3.1，不发 v2.3.2。working tree 仅剩 `testdata/`（untracked 复验素材）。CI 实证：run #30（head `39c0ca7`）三平台 success——**补丁提交推送后须重看 CI**。版本链 **2.3.1** 三处一致（pyproject / .env.example / CHANGELOG `[2.3.1] - 2026-09-05`，补丁内容已折入该段）。tag 全景：`v1.0.0`→`49b2ac6`、`v2.0.0`→`ba3ad02`、`v2.1.0`→`bd52fbb`、`v2.2.0`→`f9fb8c5`、`v2.3.0`→`e14409e`、`v2.3.1`→`39c0ca7`（重打后指向补丁收口提交）。历史重写注记与全项目状态锚点同前（`docs/health-check/PROJECT_HEALTH_CHECK.md`）。
...
| 当前质量门 | `ruff check .` 通过；`mypy src` 179 个源文件无问题；pytest 全量 **650 passed / 3 skipped / 0 failed**；`pip check` 无损坏依赖；offscreen 实测三项缺陷修复（保存/取消中文、设置下拉中文标签、筛选下拉占位语义）。 |
| 工作区 | 代码提交 `eb08c2f` 本地领先 origin/main 1 笔，**待 owner 推送**（推送后需重看 CI 三平台）；`testdata/` 保持 untracked。 |
| Remaining | **v2.3.1 补丁发版跑道（owner）**：① 推送补丁提交 → CI 三平台绿；② **tag `v2.3.1` 重打至补丁收口提交并 force 推送**（force 类操作，owner 已由"归入 v2.3.1 补丁"裁决授权机制；`git tag -f` + `git push -f origin v2.3.1`）→ release workflow 重建资产；③ **Release body 以草案 v2 重贴**（`testdata/release_body_v2.3.1.md`，已含补丁要点与质量 650/3/0）；④ 真桌面复验（P0-4/6/7/9 + 本轮两缺陷复验）；⑤ ISSUE-019 立项决策；⑥ Owner 签核。Phase E（删除语义 ADR 门）未授权。 |
| Next Step | **STOP 等待 Owner**：推送补丁提交 → CI 绿 → 重打并 force 推送 tag `v2.3.1` → 重贴 Release body → 真桌面复验 → ISSUE-019 决策 → 签核。 |
