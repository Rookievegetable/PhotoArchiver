...
| M8 可发布里程碑 | 🚧 进行中 | 用户文档已落地（installation / workflow / faq 三篇，README 增入口）；空目录 `docs/{api,design}` 已清理；**待负责人批复决策点**：License 选型 ⚠禁改清单、首版版本号与 tag、发布形态（源码分发 vs PyInstaller 需审批）、CI release workflow 增补。 |
...
| Remaining | 无已登记阻塞项。已知历史文档瑕疵：phase3-adr-draft.md §1.2 命令形状描述错误已加勘误注（草案按历史保留原文，正确契约以代码 docstring 与本文件为准）。**待负责人批复的 M8 决策点**：License 选型 ⚠、首版版本号、发布形态、release workflow 批准（详见 Next Step）。 |
| Next Step | **M8「可发布」四项决策点待批复**：① License 选定（MIT / Apache-2.0 等，批复后替换占位并同步 pyproject）；② 首版语义化版本号（建议 v1.0.0）；③ 发布形态（源码 wheel/sdist 零新依赖，或 PyInstaller 单文件 exe——后者属新增依赖须走 DEP §13 审批）；④ CI release workflow 增补批准。批复后执行：pyproject 元数据落位 → tag 与产物链路 → 发布检查单（git-rules §14）。 |
