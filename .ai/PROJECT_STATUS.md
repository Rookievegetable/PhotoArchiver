...
当前 HEAD：`1d929fe` == origin/main（LIMIT-006 skip + N3 竞态修复 + CI 事件收口均已推送），working tree 仅剩 `testdata/`（untracked 复验素材）。v2.3.1 发版终态：tag `v2.3.1`→`90c46db`（二次重打，CI 三平台绿树，remote 实测）；release workflow run #11 success；Release v2.3.1 双资产已重建（wheel 268,070 B / sdist 190,161 B @ 2026-09-06T09:47:41Z）；线上 body 尚余两处微调（667 + 构建自 90c46db）待 owner。CI 实证：run #39（head `90c46db`）三平台 success——补丁链全平台绿；LIMIT-006（macOS 压力用例 darwin skip）与 N3 竞态修复均在绿树上。版本链 **2.3.1** 三处一致（pyproject / .env.example / CHANGELOG `[2.3.1] - 2026-09-05`，补丁与人员搜索功能均已折入该段）。tag 全景：`v1.0.0`→`49b2ac6`、`v2.0.0`→`ba3ad02`、`v2.1.0`→`bd52fbb`、`v2.2.0`→`f9fb8c5`、`v2.3.0`→`e14409e`、`v2.3.1`→`90c46db`（二次重打后，remote 实测）。历史重写注记与全项目状态锚点同前（`docs/health-check/PROJECT_HEALTH_CHECK.md`）。
