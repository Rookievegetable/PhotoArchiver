...
| Remaining | **Phase D（形态一）剩余收口（owner）**：① 推送 3 笔本地提交 → **CI 三平台绿**（`eb08c2f` 触及 Presentation，必须重看）；② CI 绿后 owner 推送 tag `v2.3.1`（指向本轮收口提交，触发 release workflow）→ 核验 Release 资产与 body；③ 真桌面复验（P0-4/6/7/9，清单已交付且按钮名已同步中文化后 UI）；④ ISSUE-019 立项决策；⑤ Owner 签核发布 v2.3.1。Phase E（删除语义 ADR 门）未授权。 |
| Next Step | **STOP 等待 Owner**：推送 main 3 笔 → CI 三平台确认 → 推送 tag `v2.3.1` → 真桌面复验（中文化后 UI）→ 签核发布。 |
