...
当前 HEAD：`f8876f0` == origin/main（owner 已推送全部 6 笔 v2.3.1 补丁链），working tree 仅剩 `testdata/`（untracked 复验素材）。补丁经 owner 裁决以 tag 重打折入 v2.3.1（不发 v2.3.2）：tag `v2.3.1`→`f8876f0`（remote 实测）。CI/发版实证：CI run #30（`39c0ca7` 主链）三平台 success；release workflow run #8/`39c0ca7`、#9/`f689c01`、#10/`f8876f0` 均 success；Release v2.3.1 双资产已重建（wheel 267,654 B / sdist 189,822 B @ 16:03:29Z）、body 草案 v2 已上线。版本链 **2.3.1** 三处一致（pyproject / .env.example / CHANGELOG `[2.3.1] - 2026-09-05`，补丁与人员搜索功能均已折入该段）。tag 全景：`v1.0.0`→`49b2ac6`、`v2.0.0`→`ba3ad02`、`v2.1.0`→`bd52fbb`、`v2.2.0`→`f9fb8c5`、`v2.3.0`→`e14409e`、`v2.3.1`→`f8876f0`（补丁重打后，remote 实测）。历史重写注记与全项目状态锚点同前（`docs/health-check/PROJECT_HEALTH_CHECK.md`）。
...
| Remaining | **v2.3.1 发版技术链 ✅ 全部闭环**（tag 重打 `f8876f0` / run #9、#10 success / 资产重建 / body v2 已上线——见 §2/§3）。**真桌面复验进行中（2026-09-06 Owner 实测进度，清单 `testdata/DESKTOP_CHECKLIST_v2.3.0.md`）**：P0-4 扫描取消+单飞 ⚠️ 约 20%（待完整扫描取消测试，清单 B17–B19）；P0-6 数据安全+启动备份 ⚠️ 约 40%（待完整损坏库测试，清单 B5–B7）；P0-7 人员导入 ✅ 基础完成约 70%（Excel/TXT 导入链路已验证；待 1200 行大批量跨批用例，清单 B13/B14）；P0-9 启动路径警告 ✅ 基础完成约 80%（待部分换目录恢复测试，清单 A3 后半）；另 InsightFace 启动链路已验证通过。**自动化补充已落地（2026-09-06，Owner 批准 N1–N4 提案）**：B13/B14 大批量闭环、B19 取消一致性+重扫幂等、B7 快照恢复演练、A3 换目录子进程四项入 CI 套件（`tests/integration/test_{bulk_people_import_closed_loop,scan_cancel_consistency,backup_restore_drill,cwd_change_process_level}.py`），门禁 pytest **667 passed / 3 skipped / 0 failed**、ruff 0、mypy 180、pip check 绿；实现期实证扫描取消仅在 run 边界生效（在途取消扫完收尾，LIMIT-002 既载特征），清单 B18 预期已修正。分析全文 `testdata/AUTOMATION_ANALYSIS_v2.3.1.md`。**剩余 Owner 项**：① 真桌面复验按清单续行（自动化已消解机械部分，人工仅余感知类：视觉/原生对话框/IME/手感/指引可读性/B4 抽查）；② ISSUE-019 立项决策；③ Owner 签核发布 v2.3.1。Phase E（删除语义 ADR 门）未授权。 |
| Next Step | **STOP 等待 Owner**：真桌面复验剩余人工感知项（约 20–30 分钟抽查）→ ISSUE-019 决策 → 签核发布 v2.3.1。 |
