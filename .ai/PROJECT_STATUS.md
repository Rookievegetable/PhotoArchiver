...
- **Phase C（时序 flake 专项）**：✅ Owner 授权并解除禁修定性（2026-09-02）——F-002/LIMIT-003：`QtWorkerRunnable.is_finished` 权威终态（worker 线程直写）+ match/scan 控制器 `is_running` 语义升级（信号释放器保留为早释路径）；LIMIT-005 真因为人员轴排序未定义（Windows now() 粒度致同批 created_at 相同、UUID 决胜随机），`list_all` 决胜改 name；连续两轮全量全绿 640/3/0，两 Limit 已删除。
...
| Remaining | **Phase B 全部 5 项 + Phase C 时序 flake 专项完成（F-002/LIMIT-003、LIMIT-005 已解决删除）；Phase D（形态一）进行中：P2-5 原子写 ✅，剩余=发布验收清单执行（owner 手动，见完工报告）+ 发布说明素材（已交付会话）+ tag 命名裁决（v1.0.0 vs 沿用 2.x 递增）+ push/release**。真桌面复验清单保留待 owner 补验（P0-4/6/7/9）。Phase E（删除语义 ADR 门）未授权。发版动作（GIT-020 归 owner）：`git push origin main` 对齐 37 笔（**P0-8 CI 强校验首跑随本次推送**）。 |
| Next Step | **Phase D（形态一）完工报告已交付，STOP 等待 Owner**：① tag 命名裁决（v1.0.0 vs 沿用 2.x 递增）→ ② push（P0-8 CI 强校验首跑）→ ③ 发布验收清单手动执行 → ④ Release 创建与说明粘贴。Phase E 须另行授权。 |
...
吞吐线与历史阶段状态不变：Phase 4.2 / 5 / 6 / 7 / 8 / 9 全部 CLOSED（详见 §2）；v1.0.0 / v2.0.0 / v2.1.0 / v2.2.0 已发布。Phase D（形态一）P2-5 已完成，剩余为 owner 侧发布动作（tag 命名裁决 → push → 验收 → Release）。Phase E（删除语义 ADR 门）未授权——触发前不排期、不自动选定、发现时只记录不修复。发版收尾（非阻塞，GIT-020 归 owner）：`git push origin main` 对齐分支引用（本地领先 37 笔，含 Phase B/C 全部 + Phase D P2-5；**P0-8 CI 强校验首跑随本次推送**）。
