...
| Remaining | CURRENT_BATCH 导出（roadmap §13.12 owner 门控）· LIMIT-006 D-3（macOS 调试手段立项；D-2 已实证非 pathlib glob）· Linux CI 偶发 SIGSEGV（run #65，单次，重跑绿）观察。`migrate` 子命令与"未匹配"UI 筛选已落地（ADR-039 / ADR-036 D6 闭环），未发版（CHANGELOG Unreleased）。 |
