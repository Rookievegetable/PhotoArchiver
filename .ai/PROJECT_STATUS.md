...
- 阶段 5（识别管线吞吐加固，ADR-032）：✅ 完成——线程并行 + `add_many` 单事务批下推 + 基准工具入库（全网格基线落档；线程扩展 1.28× 弱于建议阈值）。二轮证据链闭合：batch 批推理经 W1 尖刺证据性出局；W2-segment 尖刺段内分解定址两个死重模型（landmark 35.4ms + genderage 9.8ms，生产零消费）——phase7 前置门草案已产出（`docs/development/phase7-adr-draft.md`，三项裁决点待 owner 拍板）。
...
| Remaining | 一项 owner 拍板：phase7 前置门草案三项裁决点（W2-1 landmark 死重剔除＝推荐 A `allowed_modules` 配置 / W2-2 genderage 一并剔除＝推荐 A 附条件「owner 确认无性别年龄功能规划」/ W2-3 剩余非推理段本轮不改造＝推荐 A 先复测再议）；拍板后转定稿进入执行轮（预估规模 XS-S：一行配置 + 基准复测 + 文档链，目标版本 v2.2.0）。原「W2-segment vs 止于 v2.1.0」二选一已由 owner 拍板为 A 路线，本草案即其交付物。 |
| Next Step | owner 对 phase7 草案（`docs/development/phase7-adr-draft.md` §2）拍板三项裁决点 → 转定稿并执行（loader 配置 + bench 复测 + 门禁 + 文档链 → v2.2.0）。发版项不变：v2.1.0 tag 由 owner 推送触发 release，body 建议粘贴 CHANGELOG `[2.1.0]` 段摘要（v2.1.0 发版与 phase7 拍板互不阻塞）。 |
