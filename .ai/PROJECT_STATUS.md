...
| M9 分发定位裁决 | ✅ | ADR-031 登记并执行完毕——运行形态定位为「源码/clone 唯一受支持」，Release 资产标注文案已交付 owner 粘贴；ISSUE-018 以 by-design 正式终结（KNOWN_ISSUES 回归空态）。 |
...
| Remaining | 唯一站外待办：Release v1.0.0 body 安装态标注粘贴（B5-3 owner 动作；2026-08-26 API 实测 body 首行无标注）。其余无阻塞项；CI 对 head `48c7493` 三平台矩阵实证全绿（run 32962654309，windows/ubuntu/macos 均 success，含 photopath 跨平台修复与 AI 用例路径修复）；文档三项 Minor 清偿提交 `8e350a7` 已推送。KNOWN_ISSUES 回归空态（ISSUE-018 经 ADR-031 by-design 终结，历史注记保留于文件内）。 |
| Next Step | M8/M9 收官完成（CI 三平台实证全绿 + 分发定位裁决落地，仅余 Release 标注 owner 粘贴一项站外动作）。后续候选：性能基准常态化复测；v2.0.0 清单执行（`enable(context)` 移除，见 ADR-030）；PDF 导出等依赖审批项按需触发；若出现非技术用户分发需求，按 phase5 定稿方案 B 另起前置门。 |
