...
| M9 分发定位裁决 | 🚧 前置门 | `docs/development/phase5-adr-draft.md` 产出（安装态断点 E1-E6 实证 + 方案 A/B 对比）；三项裁决点 B5-1~B5-3 待拍板（推荐 A：源码形态定位）。 |
...
| Remaining | 无阻塞项。已知历史文档瑕疵：phase3-adr-draft.md §1.2 勘误注已就位。KNOWN_ISSUES 挂 ISSUE-018（打包安装态限制）——**phase5 前置门草案已产出待拍板**（B5-1 运行形态定位 / B5-2 ISSUE 终态 / B5-3 Release 页标注，详见 Next Step）。 |
| Next Step | **M9 分发定位裁决待拍板**：阅读 `docs/development/phase5-adr-draft.md`（安装态断点 E1-E6 实证 + 方案 A 源码定位【推荐】 vs B 安装态支持）→ 批复 B5-1~B5-3 → 执行 Release 页标注与 ISSUE-018 终态化 → 收官。 |
