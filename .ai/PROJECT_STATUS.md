...
| Remaining | 无阻塞项。已知历史文档瑕疵：phase3-adr-draft.md §1.2 勘误注已就位。KNOWN_ISSUES 挂 ISSUE-018（打包安装态限制，开放至 v1.0.1 打包 ADR）。**v1.0.0 发布后 CI 三平台已全绿**（run 32937966294，含 8 个 AI 集成用例 Unix/macOS 首跑通过）。 |
| Next Step | M8 收官。后续候选：性能基准常态化复测；v1.0.1 打包策略 ADR 决策（ISSUE-018）；v2.0.0 清单执行（`enable(context)` 移除，见 ADR-030）；按用户反馈迭代（PDF 导出等需依赖审批项）。 |
