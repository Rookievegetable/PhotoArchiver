...
| Remaining | 无阻塞项。已知历史文档瑕疵：phase3-adr-draft.md §1.2 命令形状描述错误已加勘误注（草案按历史保留原文）。**唯一待办（负责人动作）**：执行 `git push origin main --follow-tags` 推送 v1.0.0 tag，待 release workflow 跑完后在 GitHub Release 页面确认 sdist/wheel 产物。 |
| Next Step | v1.0.0 发布后收尾：确认 GitHub Release 产物与 release workflow 运行日志 → 对外公告（README 徽章已就位）。后续候选：性能基准常态化复测；v2.0.0 清单执行（`enable(context)` 移除，见 ADR-030）；按用户反馈迭代（PDF 导出等需依赖审批项）。 |
