...
| Remaining | 无阻塞项。已知历史文档瑕疵：phase3-adr-draft.md §1.2 勘误注已就位。KNOWN_ISSUES 回归空态（ISSUE-018 经 ADR-031 by-design 终结，历史注记保留于文件内）。 |
| Next Step | M8/M9 收官。后续候选：性能基准常态化复测；v2.0.0 清单执行（`enable(context)` 移除，见 ADR-030）；PDF 导出等依赖审批项按需触发；若出现非技术用户分发需求，按 phase5 草案方案 B 另起前置门。 |
