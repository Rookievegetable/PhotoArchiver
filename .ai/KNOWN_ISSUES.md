...
| LIMIT-006 | macOS CI runner 原生段错误（SIGSEGV）：真实执行器扫描用例在 macOS 上于 `local_photo_file_scanner.scan` 的 pathlib 原生调用处段错误，主线程同刻停于 `qtbot.waitUntil`。已观测两次形态：① `test_scan_single_flight.py::test_real_executor_refuses_second_scan_mid_flight_and_recovers`（2000 文件压力扫描，worker 于 `pathlib.glob/_select_from`）；② `tests/integration/test_scan_cancel_consistency.py::test_rescan_grows_superset_idempotently`（N3 重扫幂等，500 文件首扫，worker 于 `pathlib.is_file/stat`，faulthandler 无 Python 异常；PySide6 6.11.1 / py3.11.9 arm64，E-5 树首次观测——崩溃在 scanner.scan 的系统调用层，早于 E-5 变更检测路径且首扫行为 E-5 前后一致，产品代码无因果）。两形态同族：真实 QThreadPool 扫描 worker + 大目录 pathlib 遍历 + 主线程 qtbot 等待的交互，时序/环境敏感非确定性（LIMIT-006 历史同日绿红交替；②同 run 同文件另一 2000 文件用例 PASSED）。CI run #32–#36（head `f689c01`→`982a3ce`，UI 缺陷修复链起）macOS 四连，同日 run #30/#31 同用例仍绿；Windows/Linux 全量绿。处置：真实执行器压力用例 darwin 范围 skipif（win/linux 照跑）——`test_scan_single_flight` / `test_scan_cancellation` 各 1 用例 + N3 `test_scan_cancel_consistency` 2 用例（预防性同范围）。开放问题：是否为 macOS runner 镜像/Qt 6.11 环境漂移与真实线程池+大目录遍历的交互，待后续轮有 macOS 调试手段时追查。**D-2 实证（2026-09-12，CI run #62/#63）**：A-2/A-3 候选部分解除——ADR-036 D5 已将扫描器从 pathlib glob 换为迭代式 os.scandir + 环检测， darwin skip 临时移除后 macOS CI Pytest 步骤仍原生段错误（exit 139 SIGSEGV），ubuntu/windows 全绿——**崩溃面不是 pathlib glob**，嫌疑收敛到"真实 QThreadPool 扫描 worker + 大目录压力 + qtbot.waitUntil"交互本身；本轮 .ips 收集为空（artifact 0），需 macOS 调试手段立项（D-3）。实验提交 05186ba/ea45c9d 已回退（b3b0d39）。**D-3 实验一（2026-09-12，CI run #73 stress-macos job）**：非 qtbot 形态（真实 QThreadPool + 2000 文件真实扫描 + 主线程 sleep 轮询，`tests/integration/test_scan_stress_no_qtbot.py`，PA_STRESS 门控）在 macOS runner **稳定通过**——QThreadPool × 大目录扫描不是充分条件，**嫌疑收敛到 qtbot.waitUntil 主线程等待交互**；PYTHONFAULTHANDLER 已三平台常开（下次崩溃可从 stderr 取 C 层栈）。**D-3 实验二（进行中）**：darwin skipif 增加 `PA_ALLOW_LIMIT_006=1` 豁免，stress-macos job 以 continue-on-error 直跑 4 个 qtbot 崩溃用例——段错误复现则 faulthandler/`.ips` 首次拿到 C 层栈，通过则 LIMIT-006 可降级为 qtbot 版本交互性环境漂移。~~降级判据（H-1）~~ **已修正**：stress streak 曾达 9 连续通过，但 run #80 实证 stress 通过 ≠ 主套件安全（6 连续通过后解除 skip 当轮即段错误）——计数器降级为 canary 遥测（403 限流已加重试退避），**skip 解除以根因修复（PySide6 降级实验 / 上游修复）为前提**，不再由 stress streak 触发。判据曾达成（6 连续通过，run #79）→ darwin skip 解除（run #80）——**主矩阵即刻段错误（exit 139）**，判据重置、skip 已恢复（stress job 聚焦跑同批用例仍通过）。**新增关键证据**：崩溃边界 = **全量套件上下文**（用例顺序/累积状态依赖），聚焦跑不触发；且 `.ips` 收集路径证实无效（DiagnosticReports 恒空）——已建新的公开取证通道：CI 失败时把 pytest 输出尾部（含 faulthandler C 层栈）逐行转 `::error::` 注解，匿名 API 可读。**D-3 突破（2026-09-12，run #80 faulthandler 栈，owner 供日志）**：崩溃点精确定位为`local_photo_file_scanner.py:71` 的 `list(os.scandir(current))`——**os.scandir 的 C 层枚举**（worker 线程 + 主线程 qtbot.waitUntil 并发时）。旧 pathlib.glob 内部同样走 scandir，故共同因子是 scandir 本身（D-5 重写无效的原因）。**实验三（进行中）**：扫描器改 `os.listdir + os.lstat` 完全绕开 scandir C 通道（2000 文件 0.140s，介于 scandir 0.037 / glob 0.218），主矩阵临时豁免 darwin skip（PA_ALLOW_LIMIT_006=1 进 job env）验证——**实验三结果（run #82）：阴性**——listdir+lstat 变体仍段错误（owner 供日志确认同栈），且 faulthandler→注解通道首次公开捕获崩溃栈。结论修正：**崩溃与枚举 API 无关**（pathlib.glob / os.scandir / os.listdir+lstat 三种实现全崩），共同因子 = **QThreadPool worker 做文件系统枚举 + 主线程 qtbot.waitUntil 事件循环**的并发形态（PySide6 6.11.1 / macOS arm64 / Python 3.11.9）——疑似 PySide6 上游缺陷。**严重性上调**：产品在 macOS 的真实扫描就是该线程形态（后台扫描 + UI 事件循环），'测试专属'的原判定不再成立。darwin skip 恢复维持。D-3 三个选项的执行：**实验四（阴性，run #88）**：PySide6 降级 6.8.3 LTS 后主矩阵豁免直跑崩溃用例——**段错误照常复现**（同栈形态），且注解捕获到新细节：worker 线程呈 `<no Python frame>`（崩溃在 Python 帧之下的原生层）。**结论：与 PySide6 版本无关**（6.8.3 与 6.11.1 均崩），降级已回滚（requirements 恢复 6.11.1，darwin skip 随豁免移除自动恢复）。**产品侧规避证伪（ADR-040 → Rejected，run #91）**：扫描移到 Python 线程后段错误复现，且崩溃点漂移至`scan_and_register_photos_service._absolute_path → pathlib.resolve → posixpath._joinrealpath`（同样 `<no Python frame>`）——线程类型不是必要条件，规避不可行，代码已回滚、darwin skip 恢复。排除矩阵终态：枚举 API（3 种实现）× 线程类型（QThreadPool/Python）× PySide6 版本（6.8.3/6.11.1）全排除，共同因子 = **后台线程的 macOS 文件系统调用 + 主线程 Qt 事件循环**——确定为本仓库外问题。**出路 = 上游 issue**（`docs/development/limit006-upstream-issue-draft.md` 证据链完整，owner 提交）；darwin skip 与注解取证通道长期维持。② 上游 issue 草稿已备（`docs/development/limit006-upstream-issue-draft.md`，owner 提交）；③ 扫描线程规避备而未动。注解通道已具备：失败时自动输出 Fatal Python error 起始的完整栈。tracker 步骤已加 continue-on-error + 异常兜底（run #80 曾因 API 故障让 stress job 误红）。 | Open | Low | 四个真实执行器压力用例在 macOS CI 跳过（其余平台照跑）；产品扫描代码路径未观测到等价崩溃形态（崩溃语境为测试专属的背靠背大压力扫描+teardown 并发）；后续轮候审。 |
